  ...
  */
  ...
  get (options, callback) {
    ...
    function query(offset) {
      request({
        ...
        timeout: options.hasOwnProperty('timeout') ? options.timeout : 10000
      }, (err, res, body) => {
        if (err) return callback(err);
        if (res.statusCode !== 200) return callback(`HTTP status code ${res.statusCode}: ${JSON.stringify(res.body)}`);
  ...
  post (options, callback) {
    ...
    request({
      ...
      if (err) return callback(err);
      if (res.statusCode !== 201) return callback(`HTTP status code ${res.statusCode}: ${JSON.stringify(res.body)}`);
  ...
  delete (options, callback) {
    ...
    request({
      ...
      if (err) return callback(err);
      if (res.statusCode !== 204) return callback(`HTTP status code ${res.statusCode}: ${JSON.stringify(res.body)}`);
      return callback(null, res);
    });
  }
}
